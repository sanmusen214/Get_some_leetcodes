...
class Solution:
    def findWords(self, board, words):
        ...
        w=len(board[0])
        result_words=[]
        board_cp=[[0 for i in range(w)] for j in range(h)]
        char_list=set([])
        for line in board:
            for c in line:
                if c not in char_list:
                    char_list.add(c)
        for wd in range(len(words)-1,-1,-1):
            for c in words[wd]:
                if(c not in char_list):
                    words.pop(wd)
                    break
        ...
        def gothis_word(nowi,nowj,oldboard,nowword,step):
            nowboard=copy.deepcopy(oldboard)
            if(board[nowi][nowj]==nowword[step]):
                nowboard[nowi][nowj]=1
                if(step==len(nowword)-1):
                    return True
                #up
                if(nowi!=0 and nowboard[nowi-1][nowj]!=1):
                    if(gothis_word(nowi-1,nowj,nowboard,nowword,step+1)):
                        return True
                #left
                if(nowj!=0 and nowboard[nowi][nowj-1]!=1):
                    if(gothis_word(nowi,nowj-1,nowboard,nowword,step+1)):
                        return True
                #right
                if(nowj!=w-1 and nowboard[nowi][nowj+1]!=1):
                    if(gothis_word(nowi,nowj+1,nowboard,nowword,step+1)):
                        return True
                #down
                if(nowi!=h-1 and nowboard[nowi+1][nowj]!=1):
                    if(gothis_word(nowi+1,nowj,nowboard,nowword,step+1)):
                        return True
            else:
                return False
        def gothisAll(i,j):
            for word_ind in range(len(words)-1,-1,-1):
                if(gothis_word(i,j,board_cp,words[word_ind],0)):
                    result_words.append(words[word_ind])
                    words.pop(word_ind)
...
a=Solution()
print(a.findWords([["b","a","b","a","b","a","b","a","b","a"],
                   ["a","b","a","b","a","b","a","b","a","b"],
                   ["b","a","b","a","b","a","b","a","b","a"],
                   ["a","b","a","b","a","b","a","b","a","b"],
                   ["b","a","b","a","b","a","b","a","b","a"],
